...
"""
#PART 1A : Using tanh activation function
...
while iteration>0:
    ...
    for x in traintokens:
        ...
        X3 = np.tanh(S3)
        #BACKPROP
        delta3 = 2*	(trainoutput[rand_pos] - X3)*(1-X3*X3)
        delta2 = (1-X2*X2)*np.dot(delta3,np.transpose(layer3))
        delta1 = (1-X1*X1)*np.dot(delta2,np.transpose(layer2))
