...
while iteration>0:
    ...
    for x in traintokens:
        ...
        X3 = sigmoid(S3)
        #BACKPROP
        delta3[0] = +4*	(trainoutput[rand_pos] - X3[0])*X3[0]*(1-X3[0])
        delta3[1] = -4*	(trainoutput[rand_pos] - X3[0])*X3[1]*(1-X3[1])
        ...
        for j in range(2):
            layer3[:,j] += learning_rate*X2*delta3[j]
    iteration-=1
    error = 0
    count = 0
    xxx   = 0
    ...
    for x in testtokens:
		    ...
		    if X3[0]>=0.5:
		        ...
		        ans = 0
		    error +=  2*((testoutput[count]-X3[0])**2) 
		    if ans == testoutput[count]:
		        xxx += 1
		    yyy += 1
		    count += 1
